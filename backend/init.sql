...
CREATE TABLE bureau_connections (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE bureau_pull_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE credit_report_changes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    client_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    bureau VARCHAR(20) NOT NULL CHECK (bureau IN ('experian', 'equifax', 'transunion')),
...
CREATE TABLE bureau_auto_pull_config (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- Bureau integration indexes
CREATE INDEX idx_bureau_connections_bureau ON bureau_connections(bureau);
CREATE INDEX idx_bureau_pull_history_client ON bureau_pull_history(client_id);
CREATE INDEX idx_bureau_pull_history_status ON bureau_pull_history(status);
CREATE INDEX idx_bureau_pull_history_created ON bureau_pull_history(created_at DESC);
CREATE INDEX idx_credit_report_snapshots_client ON credit_report_snapshots(client_id);
CREATE INDEX idx_credit_report_snapshots_client_bureau ON credit_report_snapshots(client_id, bureau, created_at DESC);
CREATE INDEX idx_credit_report_changes_client ON credit_report_changes(client_id);
CREATE INDEX idx_credit_report_changes_severity ON credit_report_changes(severity);
CREATE INDEX idx_credit_report_changes_snapshot ON credit_report_changes(snapshot_id);
CREATE INDEX idx_bureau_auto_pull_next ON bureau_auto_pull_config(next_pull_date) WHERE enabled = true;

-- ============================================================================
-- PHASE 4: COMPANY PROFILE
-- ============================================================================

-- Company information editable by admins
